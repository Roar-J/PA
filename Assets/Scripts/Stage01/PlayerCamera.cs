...
public class PlayerCamera : MonoBehaviour
{
    ...
    public IEnumerator LerpTo(Transform t, float maxTime = 2f)
    {
        float time = maxTime;
            ...
            && time > 0f)
        {
            ...
            cameraPosition = d > 1f ?
                Vector3.Lerp(cameraPosition, t.position, Time.deltaTime * 2f) : 
                Vector3.MoveTowards(cameraPosition, t.position, Time.deltaTime * 2f);
            float e = Vector3.Distance(t.eulerAngles, cameraEulerAngles);
            cameraEulerAngles = d > 1f ?
                Vector3.Lerp(cameraEulerAngles, t.eulerAngles, Time.deltaTime * 2f) : 
                Vector3.MoveTowards(cameraEulerAngles, t.eulerAngles, Time.deltaTime * 20f);
            time -= Time.deltaTime;
            yield return new WaitForEndOfFrame();
        }
    }
